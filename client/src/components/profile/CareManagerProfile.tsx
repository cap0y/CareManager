...
const CareManagerProfile = ({ user }: CareManagerProfileProps) => {
  ...
  const handleIntroImageUpload = async (id: string, file: File) => {
    ...
    try {
      ...
      if (result.success && result.imageUrl) {
        // 서버 URL과 이미지 경로 조합
        const imageUrl = normalizeImageUrl(`${serverBaseUrl}${result.imageUrl}`);
                ...
                <div className="flex flex-col items-center mb-6">
                  ...
                  <div className="relative cursor-pointer" onClick={handleAvatarClick}>
                    <Avatar className="w-24 h-24 border-4 border-purple-100">
                      <AvatarImage src={imageBase64 || normalizeImageUrl(careManager?.imageUrl || undefined) || user.photoURL || ""} />
...
💡 우측 상단의 "이미지 추가" 버튼을 사용하여 이미지를 업로드할 수 있습니다.`}
                                ...
                                <div 
                                  ...
                                  dangerouslySetInnerHTML={{ 
                                    __html: normalizeHtmlImageSrc(productForm.description || '<p class="text-gray-400">미리보기 할 내용이 없습니다. HTML 코드 탭에서 내용을 작성해주세요.</p>') 
                                  }}
                                />
                              )}
                            </div>
